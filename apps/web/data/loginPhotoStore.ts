...
const apiEndpoint = "/login-photos";
...
const migrateLegacyToServer = async (): Promise<void> => {
  ...
  const serverResponse = await apiFetch(apiEndpoint, { cache: "no-store" });
  ...
  if (Object.keys(legacyTexts).length > 0) {
    ...
    const textResponse = await apiFetch(apiEndpoint, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
...
const fetchServerStore = async (): Promise<LoginPhotoServerStore> => {
  const response = await apiFetch(apiEndpoint, { cache: "no-store" });
...
export const readLoginPhotoStore = async (): Promise<LoginPhotoServerStore> => {
  if (!readSession()) return { photos: {}, texts: {} };
