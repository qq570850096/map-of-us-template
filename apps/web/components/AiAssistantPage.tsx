...
export default function AiAssistantPage() {
  ...
  const createTripDraft = async () => {
    if (!origin.trim() || !destination.trim() || working) return;
    setWorking("trip");
    setStatus("");
    try {
  ...
  return (
    <MemoryPageShell active="ai">
      <header className="flex flex-wrap items-start justify-between gap-4 sm:gap-5">
        <div>
          <div className="flex items-center gap-3">
            <Bot className="h-6 w-6 text-[#A8C8DC] sm:h-8 sm:w-8" />
            <h1 className="text-2xl font-semibold leading-tight text-[#5A6670] sm:text-[34px]">AI 旅行助手</h1>
          </div>
      ...
      <section className="mt-6 grid gap-4 sm:mt-10 sm:gap-5 lg:grid-cols-2">
        <article className="rounded-[8px] border border-[#D8DDD8]/78 bg-[#FAFBF7]/76 p-4 shadow-[0_12px_28px_rgba(90,102,112,0.06)] sm:p-5">
        ...
        <article className="rounded-[8px] border border-[#D8DDD8]/78 bg-[#FAFBF7]/76 p-4 shadow-[0_12px_28px_rgba(90,102,112,0.06)] sm:p-5">
          ...
          <input
            ...
            placeholder="出发地，比如 郑州 / 上海虹桥 / 广州"
          />
          <input
            className="mt-3 min-h-10 w-full rounded-[7px] border border-[#D8DDD8] bg-[#FAFBF7] px-3 text-sm outline-none transition focus:border-[#E8B8C2]"
            value={destination}
          ...
          <button
            ...
            disabled={working !== null || !origin.trim() || !destination.trim()}
          >
          ...
          {tripDraft && (
            <div className="mt-5 rounded-[7px] border border-[#D8DDD8]/72 bg-white/46 p-4">
              ...
              {(tripDraft.origin || tripDraft.destination) && (
                <p className="mt-1 text-xs font-semibold text-[#5A6670]/50">
                  {tripDraft.origin || origin} → {tripDraft.destination || destination}
                </p>
              )}
