...
> 为什么会这样：应用未经过 Apple/Microsoft 的付费代码签名。想让使用者「双击零警告」打开，需要苹果开发者账号（$99/年）做签名+公证、或 Windows 代码签名证书。个人分享用上面的放行方式即可。
...
这个副本来自私人项目历史。公开发布前必须确认：

- `public/photos/login/*.jpg` 已替换为占位图或授权素材。
- 不直接推送旧 git 历史，建议用全新干净历史重建公开仓库。
- 地图审图号属于法律合规事项，公开发布前需要自行处理。
